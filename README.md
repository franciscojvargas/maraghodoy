...
- **Altura del slider:** la presentación móvil no mide en `dvh`. Esa unidad la resuelve el navegador, en móvil no siempre se recalcula tras un giro de pantalla y, con el scroll bloqueado, no hay nada que fuerce el reflow que la corregiría; como el desplazamiento del slider también iba en `dvh`, el error se multiplicaba por el número de pase y dejaba un borde negro al fondo. Ahora `useViewportHeight` publica `window.innerHeight` —un número que se lee en el momento, no una unidad diferida— en la variable `--slide-h`, y de ahí salen el alto de cada pase, el encajonado de `ClientLayout` y el recorrido del slider, ya en píxeles: los tres no pueden discrepar. El `--slide-h: 100dvh` de `globals.css` es sólo el valor del primer pintado, así que el HTML prerenderizado se ve igual que antes.
